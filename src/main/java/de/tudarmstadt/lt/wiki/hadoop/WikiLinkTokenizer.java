...
public class WikiLinkTokenizer extends Configured implements Tool {
	public boolean runJob(String inDir, String outDir) throws Exception {
		...
		job.setJarByClass(WikiLinkTokenizer.class);
		FileInputFormat.addInputPath(job, new Path(inDir));
		FileOutputFormat.setOutputPath(job, new Path(_outDir));
		job.setMapperClass(WikiLinkTokenizerMap.class);
		job.setMapOutputKeyClass(Text.class);
		job.setMapOutputValueClass(Text.class);
		job.setOutputKeyClass(Text.class);
		job.setOutputValueClass(Text.class);
		job.setNumReduceTasks(0);
		job.setJobName("WikiLinkProcessor:WikiLinkTokenizer");
	...
	public static void main(final String[] args) throws Exception {
		...
		int res = ToolRunner.run(conf, new WikiLinkTokenizer(), args);
		System.exit(res);
	}
}